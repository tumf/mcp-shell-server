...
async def test_cleanup_all_clears_and_kills(process_manager):
    ...
    with patch.object(process_manager, 'cleanup_processes', new_callable=AsyncMock) as mock_cleanup:
...
async def test_create_process_unexpected_exception():
    ...
    with patch(
        ...
        with pytest.raises(ValueError, match="Unexpected error during process creation"):
...
async def test_execute_pipeline_last_stdout_handle(process_manager):
    ...
    with patch.object(process_manager, 'create_process', new_callable=AsyncMock, return_value=mock_proc):
        with patch.object(process_manager, 'execute_with_timeout', new_callable=AsyncMock, return_value=(b"test output", b"")):
            with patch.object(process_manager, 'cleanup_processes', new_callable=AsyncMock):
...
async def test_execute_pipeline_empty_stderr_nonzero_return(process_manager):
    ...
    with patch.object(process_manager, 'create_process', new_callable=AsyncMock, return_value=mock_proc):
        with patch.object(process_manager, 'execute_with_timeout', new_callable=AsyncMock, return_value=(b"", b"")):
            with patch.object(process_manager, 'cleanup_processes', new_callable=AsyncMock):
