...
- [ ] Fix redirected stdout file-handle handling after containment is enforced. Completion condition: runtime file handles are detected with `io.IOBase` or equivalent runtime-valid checks instead of `typing.IO`. (verification: integration - `tests/test_shell_executor_redirections.py` proves valid `>` and `>>` write inside the working directory and produce expected command result metadata.)
